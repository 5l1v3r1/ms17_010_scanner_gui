...
class MyApp(object):
    ...
    def start(self, btnName):
        ...
        for x in self.ip_address_list:
            self._futures.append(self._executor.submit(checker, x))
    ...
    def wait_scan(self):
        ...
        for x in as_completed(self._futures):
            ...
            logging.info("{0:>5}/{1:<5} IP: {2:<15} RESULT:{3}".format(len(data), len(self._futures), ip, d))

        workbook = xlsxwriter.Workbook('{0}/scan_result_{1}.xlsx'.format(self.save_path, self.now))
        ...
        for item in data:
            ...
            row += 1
        workbook.close()
        # self.app.infoBox("complete", "complete!")
